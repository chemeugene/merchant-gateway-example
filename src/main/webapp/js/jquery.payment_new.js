...
(function($) {
  ...
  var methods = {
    ...
    checkControls: function() {
      ...
      if (settings.emailEnabled || settings.ofdEnabled) methods.checkControl('#' + settings.emailId);
      if (settings.phoneEnabled || settings.ofdEnabled) methods.checkControl('#' + settings.phoneId);
    ...
    showCustomFields: function() {
      if (settings.emailEnabled) {
        ...
        if (settings.ofdEnabled) {
          $('#' + settings.emailDescriptionOfd).show();
          $('#' + settings.emailDescription).hide();
        }
      }
      if (settings.phoneEnabled) {
        ...
        if (settings.ofdEnabled) {
          $('#' + settings.phoneDescriptionOfd).show();
          $('#' + settings.phoneDescription).hide();
        }
      }
    },
    customerDetails: function(data) {
      ...
      if ($('#' + settings.phoneInputId).length && data['customerDetails'].phone != null) {
        if (/^8/.test(data['customerDetails'].phone)) {
          $('#' + settings.phoneInputId).val(data['customerDetails'].phone.replace(/^8/, '+7'));
        } else if (!/^\+/.test(data['customerDetails'].phone)) {
          $('#' + settings.phoneInputId).val('+' + data['customerDetails'].phone);
        }
      }

      if (settings.hideCustomFields) {
        if (data['customerDetails'].email != null && settings.emailEnabled) {
          $('#' + settings.emailContainer).hide();
        }
        if (data['customerDetails'].phone != null && settings.phoneEnabled) {
          $('#' + settings.phoneContainer).hide();
        }
        if ((data['customerDetails'].phone != null || data['customerDetails'].email != null) && settings.ofdEnabled) {
          $('#' + settings.emailContainer).hide();
          $('#' + settings.phoneContainer).hide();
        } else {
    ...
    switchEmailEnabled: function(status) {
      if ($('#' + settings.emailInputId).length == 0) {
        settings.emailEnabled = false;
      } else {
        settings.emailEnabled = status;
      }
    },
    switchPhoneEnabled: function(status) {
      if ($('#' + settings.phoneId).length == 0) {
        settings.phoneEnabled = false;
      } else {
        settings.phoneEnabled = status;
      }
    },
    ...
    validatePan: function() {
      function switchPanValidClasses(valid) {
        if (settings.visualValidationEnabled) {
          classToRemove = valid ? "invalid" : "valid";
          classToAdd = valid ? "valid" : "invalid";
          $("#" + settings.panInputId).removeClass(classToRemove);
          $("#" + settings.pan1InputId).removeClass(classToRemove);
          $("#" + settings.pan2InputId).removeClass(classToRemove);
          $("#" + settings.pan3InputId).removeClass(classToRemove);
          $("#" + settings.pan4InputId).removeClass(classToRemove);
          $("#" + settings.panInputId).addClass(classToAdd);
          $("#" + settings.pan1InputId).addClass(classToAdd);
          $("#" + settings.pan2InputId).addClass(classToAdd);
          $("#" + settings.pan3InputId).addClass(classToAdd);
          $("#" + settings.pan4InputId).addClass(classToAdd);
        }
      }
    ...
    validatePhone: function() {
      ...
      if ($('#' + settings.phoneId).val().trim() === "" ||
        ...
        properties.validatePhone = true;
        $("#" + settings.phoneId).removeClass("invalid");
        $("#" + settings.phoneId).addClass("valid");
      } else {
        properties.validatePhone = false;
        $("#" + settings.phoneId).removeClass("valid");
        $("#" + settings.phoneId).addClass("invalid");
      }
    },
    validateOfdCustomFields: function() {
      if (settings.ofdEnabled) {
        var validate = false;
        if (
          ($('#' + settings.emailId).val().split(" ").join("") !== "" && properties.validateEmail) ||
          ($('#' + settings.phoneId).val().trim() !== "" && properties.validatePhone)
        ) {
          validate = true;
        }
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          if ('payerNotificationEnabled' in data) {
            if (data['payerNotificationEnabled'] && settings.emailEnabled) {
              $('#' + settings.emailDescription).show();
            }
          }
